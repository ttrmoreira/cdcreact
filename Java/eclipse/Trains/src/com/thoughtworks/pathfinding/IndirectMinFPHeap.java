...
public class IndirectMinFPHeap{ 
  
	private double p[]; 
	private int n, pos[], fp[];    
  
  /*The class IndirectMinFPHeap presents data structures and operations
   * needed for operate with a indirect heap.*/
  public IndirectMinFPHeap (double p[], int v[]) { 
  ...
  public void reDo (int esq, int dir) {
    int j = esq * 2;
    int x = this.fp[esq];
    while (j <= dir) {
  ...
  public void build () {
    int esq = n / 2 + 1;
    while (esq > 1) { esq--; this.reDo (esq, this.n); }
  }
 
  public int withdrawsMin () throws Exception {
    int minimo;
    if (this.n < 1) throw new Exception ("Erro: heap vazio");
    else {
      minimo = this.fp[1]; this.fp[1] = this.fp[this.n];
      this.pos[fp[this.n--]] = 1; this.reDo (1, this.n);
    }
    return minimo;
  }

  public void keyDecreases (int i, double chaveNova) throws Exception {
